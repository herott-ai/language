...
注意：**==一个字符串赋值给一个变量时，如果没有双引号或单引号，那么这个字符串也一定不能有空格==**。
...
**==上图为静默模式下不显式输出换行的打印。注意：这其中输入完成后我是按了回车建的哦==**。
...
**==这个返回0代表真，返回1代表假(貌似它就不是根据返回值来判断的)。没必要过多的纠结返回值，我们`只看这个表达式的含义`然后配合if语句使用即可==**。
