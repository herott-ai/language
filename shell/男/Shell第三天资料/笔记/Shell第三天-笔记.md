...
获取第一个字符在字符串中出现的位置语法(**==字符串下标是从1开始计数的==**)
...
![1640790676769](assets/1640790676769.png)
...
![1640784607879](assets/1640784607879.png)
...
**==如果有非常多表达式需要计算，我们可以放到文件中再用bc进行计算。只是结果不能放到shell中==**。
...
==**所以，这个函数的功能是去除`/字符串`。你给出`a.out`就只剩一个`.`**==。
