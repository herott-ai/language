...
const:只读的（对）  常量（错）（或说半错半对）

对于非指针，const int =int const,两者无区别

对于指针，有三种情况const int * p,    int const * p,    int * const p。p为指针变量

我们去掉int,会发现前两种情况一样。

前一种情况是p指向的空间是只读的，不可修改。也就是如果p=&a,那么不可通过解引用去修改a的值。常量指针

后一种情况是p自身的空间是只读的，不可修改。指针常量

p是一个变量。p=&a就跟a=10一样(const int a=10)。a不可改为20，同样，p不可再取变量b的地址
...
int main() {
	...
	//const修饰的是指针*(指针指向的空间)，指针指向可以改，指针指向的值不可以更改
	const int * p1 = &a; 
	p1 = &b; //正确
	//*p1 = 100;  报错
